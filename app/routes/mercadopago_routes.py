...
async def create_preference_cart(
    ...
    """
    Cria uma preferência de pagamento no Mercado Pago para um carrinho de compras inteiro.
    """
    ...
    # --- Valida cada item do carrinho e constrói a lista para o MP ---
    for item in cart.items:
        ...
        # Novo formato da referência: "id_produto1,qtde1;id_produto2,qtde2"
        external_reference_parts.append(f"{item.product_id},{item.quantity}")

    # --- Configuração das URLs e Referência Externa ---
    base_url = os.getenv("FRONTEND_URL", "http://localhost:4200")
    
    # Formato final da referência: "user_id:id1,qt1;id2,qt2:uuid_unico"
...
async def mercadopago_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    # ... (a mágica acontece na função 'process_successful_payment' que é chamada por ela)
...
def process_successful_payment(db: Session, payment_info: dict):
    """
    Função isolada e transacional para processar um pagamento aprovado.
    AGORA MODIFICADA PARA PROCESSAR MÚLTIPLOS ITENS.
    """
